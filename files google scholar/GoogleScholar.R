...
Data1 <- read.csv("PoPCites2.csv", header = T, encoding = "UTF-8")
Data2 <- read.csv("PoPCites1.csv", header = T, encoding = "UTF-8")
...
Data3 <- mutate_each(Data3, funs(toupper))
#dfinal <- rbind(Data2, Data3, deparse.level=)
Data3$AU <- gsub(", ", ";", str_trim(Data3$AU))

#options(stringsAsFactors = FALSE)
#Data3$AU <- as.vector(gsub(Data3$AU, pattern = "^.*[.,•]{3}\\s*(.*)", replace = "\\1"))
...
lines <- stri_read_lines("PoPCites2.csv", encoding = "auto")
Data6 <- read.csv("PoPCites2.csv", header = T, encoding = "UTF-8")
